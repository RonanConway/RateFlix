...
func (r *Registry) Register(ctx context.Context, instanceID string, serviceName string, hostPort string) error {
	...
	return r.client.Agent().ServiceRegister(&consul.AgentServiceRegistration{Address: parts[0], ID: instanceID,
		Name: serviceName, Port: port, Check: &consul.AgentServiceCheck{CheckID: instanceID, TTL: "5s"}})
}
...
func (r *Registry) ServiceAddresses(ctx context.Context, serviceName string) ([]string, error) {
	...
	for _, e := range entries {
		...
		if address == "" {
			address = e.Node.Address // fallback if Service.Address is empty
		}
		res = append(res, fmt.Sprintf("%s:%d", address, e.Service.Port))
	}
